...
## Introduction
cron-job.org provides an API that enables users to programmatically create, update, delete, and view cron jobs. The API provides a REST-like interface with API-key-based authorization and JSON request and response payloads. Given this, the API should be easily usable from virtually any programming language.

## Limitations
To prevent abuse of the service, cron-job.org enforces a daily usage limit. By default, this limit is 100 requests per day, but it can be increased upon request. For sustaining members, a higher limit of 5,000 requests per day applies.

Apart from the daily request limit, individual rate limits might apply depending on the specific API call made. Those limits are mentioned in the documentation for each API method.

## Requests

### Endpoint
The API endpoint is reachable via HTTPS at [https://api.cron-job.org/](https://api.cron-job.org/).

### Authentication
All requests to the API must be authenticated via an API key supplied in the `Authorization` bearer token header. API keys can be retrieved or generated in the [cron-job.org Console](https://console.cron-job.org) under “Settings”. An example header could look like:

```bash
Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ
```

Access via a specific API key might be restricted to certain IP addresses if configured in the Console. In this case, requests from non-allowlisted IP addresses will be rejected with HTTP status code `403`.

> **Warning:** API keys are secrets, just like a password. They allow access to your cron-job.org account via the API and should always be treated confidentially. It is highly recommended to also enable IP address restriction whenever possible.

### Content Type
If a request requires a payload, it must be JSON-encoded and the `Content-Type` header must be set to `application/json`. If the `Content-Type` header is missing or contains a different value, the request payload will be ignored.
...
```bash
curl -X PATCH      -H 'Content-Type: application/json'      -H 'Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ'      -d '{"job":{"enabled":true}}'      https://api.cron-job.org/jobs/12345
```
...
| --- | --- | --- |
| jobs | array of Job | List of jobs present in the account |
...
```bash
curl -X GET      -H 'Content-Type: application/json'      -H 'Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ'      https://api.cron-job.org/jobs
```
...
| Key | Type | Description |
| --- | --- | --- |
| jobDetails | array of DetailedJob | Job details |

**curl Example**

```bash
curl -X GET      -H 'Content-Type: application/json'      -H 'Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ'      https://api.cron-job.org/jobs/12345
```
...
| Key | Type | Description |
| --- | --- | --- |
| job | DetailedJob | Job (only the `url` field is mandatory) |
...
```bash
curl -X PUT      -H 'Content-Type: application/json'      -H 'Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ'      -d '{"job":{"url":"https://example.com","enabled":true,"saveResponses":true,"schedule":{"timezone":"Europe/Berlin","expiresAt":0,"hours":[-1],"mdays":[-1],"minutes":[-1],"months":[-1],"wdays":[-1]}}}'      https://api.cron-job.org/jobs
```
...
| Key | Type | Description |
| --- | --- | --- |
| job | DetailedJob | Job delta (only include changed fields - unchanged fields can be left out) |
...
```bash
curl -X DELETE      -H 'Content-Type: application/json'      -H 'Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ'      https://api.cron-job.org/jobs/12345
```
...
| Key | Type | Description |
| --- | --- | --- |
| history | array of HistoryItem | The last execution history items |
| predictions | array of int | Unix timestamps (in seconds) of the predicted next executions (up to 3) |

Please note that the `headers` and `body` fields of the `HistoryItem` objects will not be populated. To retrieve headers and body, see Retrieving Job Execution History Item Details.

**curl Example**

```bash
curl -X GET      -H 'Content-Type: application/json'      -H 'Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ'      https://api.cron-job.org/jobs/12345/history
```
...
```json
{
  ...
  "predictions": [1640190600, 1640191500, 1640192400]
}
```
...
| Key | Type | Description |
| --- | --- | --- |
| jobHistoryDetails | HistoryItem | History item |

**curl Example**

```bash
curl -X GET      -H 'Content-Type: application/json'      -H 'Authorization: Bearer zaX78aqKJuIH4l4RX6njoqADn77MQNJJ'      https://api.cron-job.org/jobs/12345/history/12345-22-11-4946
```
...
| Key | Type | Description | Default |
| --- | --- | --- | --- |
| timezone | string | Schedule time zone (see [here](https://www.php.net/manual/timezones.php) for a list of supported values) | `UTC` |
...
| appConnect | int | Time from transfer start until SSL handshake completed (in microseconds) - `0` if not using SSL |
