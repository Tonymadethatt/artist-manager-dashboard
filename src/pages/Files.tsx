    ...
    <div className="space-y-4 max-w-3xl">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border-2 border-dashed border-neutral-200 rounded-lg">
          <FilesIcon className="h-8 w-8 text-neutral-300 mx-auto mb-3" />
          <p className="font-medium text-neutral-600 text-sm mb-1">No files yet</p>
          <p className="text-xs text-neutral-400 mb-4">Generate a file from a template to see it here.</p>
        ...
        <div className="rounded border border-neutral-200 overflow-hidden bg-white">
          <table className="w-full text-sm">
            ...
            <tbody>
              {files.map(file => (
                <tr key={file.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50 transition-colors">
                  <td className="px-4 py-3">
                    <span className="font-medium text-neutral-900">{file.name}</span>
                  </td>
                  <td className="px-3 py-3 hidden sm:table-cell">
                    {file.venue ? (
                      <span className="text-xs text-neutral-600">{file.venue.name}</span>
                    ) : (
      ...
      {preview && (
        <>
          <div className="fixed inset-0 z-40 bg-black/20" onClick={() => setPreview(null)} />
          <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-lg bg-white border-l border-neutral-200 flex flex-col shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-200">
              <div>
                <h2 className="font-semibold text-sm text-neutral-900">{preview.name}</h2>
                <p className="text-xs text-neutral-400 mt-0.5">{new Date(preview.created_at).toLocaleString()}</p>
              </div>
            ...
            <pre className="flex-1 overflow-y-auto p-5 text-xs font-mono text-neutral-700 whitespace-pre-wrap leading-relaxed">
              {preview.content}
            </pre>
          </div>
        </>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg border border-neutral-200 p-6 max-w-sm w-full shadow-xl">
            <h3 className="font-semibold text-neutral-900 mb-2">Delete file?</h3>
