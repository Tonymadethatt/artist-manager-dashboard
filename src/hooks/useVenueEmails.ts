...
export function useVenueEmails() {
  ...
  const fetchEmails = useCallback(async (options?: FetchVenueEmailsOptions) => {
    ...
    const { data, error } = await supabase
      .from('venue_emails')
      .select('*, venue:venues(id, name, city, location), deal:deals(id, description, event_date, event_start_at, event_end_at, gross_amount, agreement_url, agreement_generated_file_id, venue_id, notes, payment_due_date, artist_paid, deposit_paid_amount, balance_paid_amount, deposit_due_amount, pricing_snapshot), contact:contacts(id, name, email)')
  ...
  const updateEmailStatus = async (
    id: string,
    status: VenueEmailStatus,
    patch?: { notes?: string | null; resend_message_id?: string | null },
  ) => {
    const sentAt = status === 'sent' ? new Date().toISOString() : null
    const rid =
      patch?.resend_message_id !== undefined
        ? (patch.resend_message_id?.trim() || null)
        : undefined
    const { error } = await supabase
      ...
      .update({
        ...
        ...(rid !== undefined ? { resend_message_id: rid } : {}),
      })
      .eq('id', id)
    if (error) return { error: new Error(error.message) }
