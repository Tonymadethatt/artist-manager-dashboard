...
export function buildRetainerReceivedEmailHtml(
  ...
  const feeTableBlock = settledFees.length > 0
    ...
    <div style="background:#1a1a1a;border:1px solid #2a2a2a;border-radius:8px;overflow:hidden;margin-bottom:20px;">
      <div style="background:#161616;padding:10px 18px;border-bottom:1px solid #2a2a2a;">
        ...
        <span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.4px;color:#888888;vertical-align:middle;">Retainer received in full</span>
      </div>
    ...
    : `
    <p style="font-size:14px;color:#888888;line-height:1.75;margin-bottom:24px;">Your retainer is all caught up on our side. Thank you.</p>`
  ...
  const totalCallout = settledFees.length > 0
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.2);border-radius:8px;padding:18px 22px;margin:4px 0 24px;">
      <div style="font-size:13px;color:#d1d1d1;">Total acknowledged</div>
...
<div class="wrapper" style="max-width:600px;margin:24px auto;background:#111111;border-radius:10px;overflow:hidden;border:1px solid #2a2a2a;">
  ...
  <div class="email-body" style="padding:28px 32px;">
    ...
    : defaultRecap}</p>
    <p style="font-size:14px;color:#d1d1d1;line-height:1.8;margin-bottom:28px;">Here is a quick confirmation for your records:</p>
    ...
    : `<p style="font-size:14px;color:#d1d1d1;line-height:1.8;margin-bottom:12px;">Thanks again. If anything looks off on your side, reply and we will sort it.</p>
    <p style="font-size:14px;color:#d1d1d1;line-height:1.8;">${escapeHtmlEnt(managerName)}</p>`}
  </div>
  ...
  <div class="email-footer" style="background:#0a0a0a;border-top:1px solid #1e1e1e;padding:20px 32px;">
    ...
    ${(profile.website || handle || profile.phone) ? `<div style="margin-top:10px;display:flex;align-items:center;flex-wrap:wrap;gap:0;">${[
      profile.website ? `<a href="${escapeHtmlEnt(profile.website)}" style="color:#888888;text-decoration:none;font-size:11px;">${escapeHtmlEnt(profile.website.replace(/^https?:\/\//, ''))}</a>` : '',
      handle ? `<a href="https://instagram.com/${escapeHtmlEnt(handle)}" style="display:inline-flex;align-items:center;gap:4px;text-decoration:none;vertical-align:middle;"><img src="${igIconUrl}" alt="IG" width="13" height="13" style="display:inline-block;vertical-align:middle;opacity:0.6;" /><span style="font-size:11px;color:#888888;">@${escapeHtmlEnt(handle)}</span></a>` : '',
      profile.phone ? `<span style="font-size:11px;color:#888888;">${escapeHtmlEnt(profile.phone)}</span>` : '',
