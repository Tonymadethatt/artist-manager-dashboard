...
import {
  formatPacificStackLineFromYmd,
  type ScheduleWhenStack,
  scheduleWhenStackFromDeal,
  scheduleWhenStackFromYmd,
...
export interface VenueRenderDeal {
  ...
  notes: string | null
  /** When set, stacked “when” cells match artist templates (event window + optional set window). */
...
function rowStackedInner(stack: ScheduleWhenStack, valueColor: string): string {
  return stackedScheduleWhenCellHtml(stack, valueColor, 'compact')
}

/** YYYY-MM-DD only (e.g. due date): same stacked layout as artist templates. */
function rowStackedYmd(label: string, ymd: string | null | undefined, valueColor = EMAIL_TEXT_PRIMARY): string {
  const t = ymd?.trim()
  if (!t) return ''
  const stack = scheduleWhenStackFromYmd(t)
  if (!stack) {
    return row(label, formatPacificStackLineFromYmd(t), valueColor)
  }
  const inner = rowStackedInner(stack, valueColor)
...
/**
 * Event / show date from full deal — uses event_start/end and performance window when present
 * (same `scheduleWhenStackFromDeal` path as artist gig emails).
 */
function rowStackedDateFromDeal(
  label: string,
  deal: VenueRenderDeal | undefined,
  valueColor = EMAIL_TEXT_PRIMARY,
): string {
  if (!deal) return ''
  const stack =
    scheduleWhenStackFromDeal({
    ...
    }) ?? (deal.event_date?.trim() ? scheduleWhenStackFromYmd(deal.event_date.trim()) : null)
  if (!stack) {
    const ymd = deal.event_date?.trim()
    return ymd ? row(label, formatPacificStackLineFromYmd(ymd), valueColor) : ''
  }
  const inner = rowStackedInner(stack, valueColor)
  return `<table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;"><tr><td style="padding:10px 0;font-size:13px;color:${EMAIL_ROW_LABEL};border-bottom:1px solid #222222;vertical-align:top;">${label}</td><td style="padding:10px 0;text-align:right;padding-left:16px;border-bottom:1px solid #222222;vertical-align:top;">${inner}</td></tr></table>`
}
...
export function buildVenueEmailDocument(opts: BuildVenueEmailDocumentOptions): string {
  ...
  switch (type) {
    case 'booking_confirmation': {
      ...
      const dealRows = [
        deal ? rowStackedDateFromDeal('Event date', deal, '#ffffff') : '',
    ...
    case 'payment_reminder': {
      ...
      const reminderRows = [
        ...
        deal?.payment_due_date ? rowStackedYmd('Due date', deal.payment_due_date, '#ef4444') : '',
      ].filter(Boolean).join('')
    ...
    case 'rebooking_inquiry': {
      ...
      const rebookDetails = [
        deal ? rowStackedDateFromDeal('Previous event date', deal, '#ffffff') : '',
        row('Venue', venueName, '#ffffff'),
      ].filter(Boolean).join('')
